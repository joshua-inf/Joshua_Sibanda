      ...
      <div className="container max-w-4xl mx-auto">
        ...
        <header className="mb-10 pb-8 border-b border-slate-800">
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {project.technologies.map((tech) => (
              <span
                key={tech}
                className="px-3 py-1 bg-slate-800 text-slate-300 rounded-md text-xs font-semibold border border-slate-700"
              >
                {tech}
              </span>
            ))}
          ...
          <div className="flex gap-4">
            ...
            {project.githubUrl && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="px-5 py-2.5 bg-slate-850 hover:bg-slate-800 text-slate-200 hover:text-white rounded-lg border border-slate-700 transition-all font-medium text-sm inline-flex items-center gap-2"
              >
                💻 View Source Code
              </a>
            )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-8">
            <div>
              ...
              <p className="text-slate-350 leading-relaxed text-lg whitespace-pre-line">
            ...
            {project.features && project.features.length > 0 && (
              <div>
                <h2 className="text-xl font-bold text-white mb-4">Key Features & Milestones</h2>
          ...
          {/* Sidebar Metadata */}
          <div className="bg-slate-850 p-6 rounded-lg border border-slate-800 h-fit space-y-6">
            <div>
              <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-2">Platform</h3>
              <p className="text-white font-medium capitalize">{project.category.replace("-", " ")}</p>
            </div>
            <div>
              <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-2">Scope</h3>
              <p className="text-white font-medium">Design, Frontend, Backend & Database Architecture</p>
            </div>
          </div>
        </div>
        ...
        {project.screenshots && project.screenshots.length > 0 && (
          <div className="mt-16 pt-10 border-t border-slate-800">
            <h2 className="text-xl font-bold text-white mb-6">Screenshots & Walkthrough</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {project.screenshots.map((screenshot, index) => (
                <div
                  key={index}
                  className="rounded-lg overflow-hidden border border-slate-850 hover:border-slate-700 transition-colors select-none"
                >
                  <img
                    src={screenshot}
